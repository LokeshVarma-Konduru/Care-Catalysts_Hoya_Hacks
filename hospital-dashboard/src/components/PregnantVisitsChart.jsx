...
const PregnantVisitsChart = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/analysis/pregnant-visits');
      setData(response.data);
      setError(null);
    } catch (error) {
      console.error('Error fetching pregnant visits data:', error);
      setError('Failed to load pregnant visits data');
    } finally {
      setLoading(false);
    }
  };
    ...
    datasets: [
      {
        ...
        data: data.visits.map(v => v.Pregnant_Visits_Before),
      ...
      },
      {
        ...
        data: data.visits.map(v => v.Pregnant_Visits_After),
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Pregnant Women Visits Trend Analysis',
        align: 'start',
        font: {
          size: 16,
          weight: 'bold'
  ...
  return (
    <Paper elevation={2} sx={{ p: 3 }}>
      <Typography variant="h5" gutterBottom sx={{ mb: 3 }}>
        Pregnant Women Visits Analysis
      </Typography>
...
export default PregnantVisitsChart;
